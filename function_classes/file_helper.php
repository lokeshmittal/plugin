...
final class DIVI_HELPER {

    static $notices = array();

    //log test data while makes debbuging
    public static function log($string) {
	$handle = fopen(DIVI_PATH . 'log.txt', 'a+');
	$string .= PHP_EOL;
	...
	//***
	if (isset($DIVI->settings['cache_terms']) AND $DIVI->settings['cache_terms'] == 1) {
	    $cache_key = 'divi_terms_cache_' . md5($taxonomy . '-' . (int) $hide_empty . '-' . (int) $get_childs . '-' . (int) $selected . '-' . (int) $category_parent . '-' . $lang_key);
	...
	$orderby = apply_filters('divi_get_terms_orderby', $taxonomy);
	if (!empty($orderby)) {
	    if ($orderby != $taxonomy) {
		$args['orderby'] = $orderby;
	    }
	}
	$order = apply_filters('divi_get_terms_order', $taxonomy, $orderby);
	if (!empty($order)) {
	    if ($order != $taxonomy) {
		$args['order'] = $order;
	    }
	...
	if (!empty($cats_objects)) {
		...
		if (is_object($value) AND $value->category_parent == $category_parent) {
		    ...
		    if (isset($DIVI->settings['non_latin_mode']) AND $DIVI->settings['non_latin_mode'] == 1) {
	...
	if (isset($DIVI->settings['cache_terms']) AND $DIVI->settings['cache_terms'] == 1) {
	    ...
	    if (isset($DIVI->settings['cache_terms_auto_clean'])) {
		$period = $DIVI->settings['cache_terms_auto_clean'];
	...
	if (empty($string)) {
		...
		$string = $DIVI->settings['custom_tax_label'][$taxonomy_info->name];
	    }
	}
	...
	if (class_exists('SitePress')) {
	    ...
	    $divi_settings = get_option('divi_settings');
	    if (isset($divi_settings['wpml_tax_labels']) AND ! empty($divi_settings['wpml_tax_labels'])) {
		$translations = $divi_settings['wpml_tax_labels'];
	...
	global $_chosen_attributes, $wpdb, $wp, $DIVI;
	 ...
	 */
	$min_price = $DIVI->is_isset_in_request_data('min_price') ? esc_attr($request['min_price']) : '';
	$max_price = $DIVI->is_isset_in_request_data('max_price') ? esc_attr($request['max_price']) : '';
	...
	$price_slider_html = '<form method="get" action="' . esc_url($form_action) . '">
			<div class="price_slider_wrapper">
				...
				<div class="price_slider_amount">
					<input type="text" id="min_price" name="min_price" value="' . esc_attr($min_price) . '" data-min="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $min)) . '" placeholder="' . __('Min price', 'products-filter-custom') . '" />
					<input type="text" id="max_price" name="max_price" value="' . esc_attr($max_price) . '" data-max="' . esc_attr(apply_filters('woocommerce_price_filter_widget_amount', $max)) . '" placeholder="' . __('Max price', 'products-filter-custom') . '" />
					<button type="submit" class="button">' . __('Filter', 'products-filter-custom') . '</button>
					<div class="price_label" style="display:none;">
						' . __('Price:', 'products-filter-custom') . ' <span class="from"></span> &mdash; <span class="to"></span>
					</div>
					' . $fields . '
					<div class="clear"></div>
				</div>
	...
	$price_slider_html = apply_filters('divi_price_slider_html', $price_slider_html, $price_slider_data);
	...
	if (isset($divi_settings['by_price']['ranges']) AND ! empty($divi_settings['by_price']['ranges'])) {
	    global $DIVI;
	    $res = array();
	    $request = $DIVI->get_request_data();
	    $res['selected'] = '';
	    ...
	    $ranges = explode(',', trim($divi_settings['by_price']['ranges']));
	    $get = $request;
	    $max = self::get_max_price();
		 ...
		 */
		//$_REQUEST['divi_price_filter_working'] = TRUE;
		...
		//$_REQUEST['divi_price_filter_working'] = FALSE;
		$value = $tmp[0] . ' - ' . $tmp[1];
		//***
		$v = explode('-', $key);
		$_GET['min_price'] = $v[0];
		...
		$r[$key] = $value;
		if ($show_count) {
		    $rc[$key] = $DIVI->dynamic_count(NULL, 'none', $additional_taxes);
		} else {
		    $rc[$key] = 0;
		}
	    }
	...
	if (isset($_REQUEST['divi_wp_query_ids'])) {
		...
		WC()->query->layered_nav_product_ids = $_REQUEST['divi_wp_query_ids'];
	    }
	}
    }
     ...
     * @return int
     * woocommerce native function from 2.6.0 version - added by DIVI author
     */
...
// Fix for price slider in ajax
	if (isset($_REQUEST['divi_wp_query'])AND ! empty($_REQUEST['divi_wp_query'])) {
	    $arg = $_REQUEST['divi_wp_query'];
	...
	global $DIVI;
	echo $DIVI->render_html(DIVI_PATH . 'display_data/notices/' . $key . '.php');
    }

    public static function add_notice($key) {
	//update_option('divi_notices', array());
	$notices = get_option('divi_notices', array());
	$is_hidden = false;
	...
	if (!$is_hidden) {
	    self::$notices[] = $key;
	    $notices[] = $key;
	    update_option('divi_notices', $notices);
	}
    }

    public static function hide_admin_notices() {
	if (isset($_GET['divi_hide_notice']) && isset($_GET['_wpnonce'])) {
	    if (!wp_verify_nonce($_GET['_wpnonce'])) {
		wp_die(__('Action failed. Please refresh the page and retry.', 'products-filter-custom'));
	    }

	    if (!current_user_can('manage_woocommerce')) {
		wp_die(__('Cheatin&#8217; huh?', 'products-filter-custom'));
	    }

	    $key = self::escape($_GET['divi_hide_notice']);
	...
	global $DIVI;
	$condition = 'closed';
	$toggle_type = ((isset($DIVI->settings['toggle_type']) AND ! empty($DIVI->settings['toggle_type'])) ? $DIVI->settings['toggle_type'] : 'text');


	if ($block_is_closed) {
		/*$toggle_text = ((isset($DIVI->settings['toggle_closed_text']) AND ! empty($DIVI->settings['toggle_closed_text'])) ? self::wpml_translate(null, $DIVI->settings['toggle_closed_text']) : '∨'); */
	    $toggle_text = ((isset($DIVI->settings['toggle_closed_text']) AND ! empty($DIVI->settings['toggle_closed_text'])) ? self::wpml_translate(null, $DIVI->settings['toggle_closed_text']) : '');
	    $toggle_image = ((isset($DIVI->settings['toggle_closed_image']) AND ! empty($DIVI->settings['toggle_closed_image'])) ? $DIVI->settings['toggle_closed_image'] : DIVI_LINK . 'img/plus3.png');
	} else {
	    $toggle_text = ((isset($DIVI->settings['toggle_opened_text']) AND ! empty($DIVI->settings['toggle_opened_text'])) ? self::wpml_translate(null, $DIVI->settings['toggle_opened_text']) : '');
	    $toggle_image = ((isset($DIVI->settings['toggle_opened_image']) AND ! empty($DIVI->settings['toggle_opened_image'])) ? $DIVI->settings['toggle_opened_image'] : DIVI_LINK . 'img/minus3.png');
	    $condition = 'opened';
	}
	...
	if ($toggle_type == 'text' OR empty($toggle_image)) {
	    ?>
	    <a href="javascript: void(0);" title="<?php _e('toggle', 'products-filter-custom') ?>" class="divi_front_toggle divi_front_toggle_<?php echo $condition ?>" data-condition="<?php echo $condition ?>"><?php echo $toggle_text ?></a>
	    <?php
	} else {
	    ?>
	    <a href="javascript: void(0);" title="<?php _e('toggle', 'products-filter-custom') ?>" class="divi_front_toggle divi_front_toggle_<?php echo $condition ?>" data-condition="<?php echo $condition ?>">
	        <img src="<?php echo $toggle_image ?>" alt="<?php _e('toggle', 'products-filter-custom') ?>" />
	    </a>
	    <?php
	}
    }
	...
	$args = apply_filters('divi_get_more_less_button_' . $type, array());
	if (empty($args)) {
	    $args['type'] = 'text'; //image
	    $args['closed'] = __('Show more', 'products-filter-custom');
	    $args['opened'] = __('Show less', 'products-filter-custom');
	}

	if ($args['type'] == 'image') {
	    ?>
	    <a href="javascript:void(0);" class="divi_open_hidden_li_btn" data-type="<?php echo $args['type'] ?>" data-state="closed" data-closed="<?php echo $args['closed'] ?>" data-opened="<?php echo $args['opened'] ?>"><img src="<?php echo $args['closed'] ?>" alt="" /></a>
	    <?php
	} else {
	    ?>
	    <a href="javascript:void(0);" class="divi_open_hidden_li_btn" data-type="<?php echo $args['type'] ?>" data-state="closed" data-closed="<?php echo $args['closed'] ?>" data-opened="<?php echo $args['opened'] ?>"><?php echo $args['closed'] ?></a>
	    <?php
	}
    }
