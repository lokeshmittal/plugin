...
abstract class DIVI_EXT {
    ...
    public $index = NULL; //for by_html_type only: 'divi_sku' for example. This is key in the link
    ...
    {
        $this->divi_settings = get_option('divi_settings', array());
        ...
        if ($this->type === NULL)
        {
            wp_die('DIVI EXTENSION TYPE SHOULD BE DEFINED!');
        }
    ...
    {
        return $this->get_ext_path() . 'display_data' . DIRECTORY_SEPARATOR . 'divi.php';
    }

    public function print_html_type()
    {
        global $DIVI;
        echo $DIVI->render_html($this->get_html_type_view());
    }
    ...
    public static function draw_options($options, $folder_name = '')
    {
        ...
        {
            echo $DIVI->render_html(DIVI_PATH . 'display_data' . DIRECTORY_SEPARATOR . 'ext_options.php', array(
                'options' => $value,
                'key' => $key,
                'divi_settings' => $DIVI->settings
                    )
            );
        }
    }
    ...
    public static function is_ext_activated($full_path)
    {
        ...
        if (isset($divi_settings['activated_extensions']))
        {
            $checked1 = in_array($idx1, (array) $divi_settings['activated_extensions']);
            $checked2 = in_array($idx2, (array) $divi_settings['activated_extensions']);
        }
        //$checked=false;
