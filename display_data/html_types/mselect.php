...
if (!function_exists('divi_draw_mselect_childs'))
{

    function divi_draw_mselect_childs(&$collector, $taxonomy_info, $term_id, $tax_slug, $childs, $level, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos)
    {
        ...
        $childs = apply_filters('divi_sort_terms_before_out', $childs, 'mselect');
        ?>
        <?php if (!empty($childs)): ?>
            <?php foreach ($childs as $term) : ?>
                ...
                {
                    divi_draw_mselect_childs($collector, $taxonomy_info, $term['term_id'], $tax_slug, $term['childs'], $level + 1, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos);
                }
                ?>
            <?php endforeach; ?>
        <?php endif; ?>
...
?>
<select class="divi_mselect divi_mselect_<?php echo $tax_slug ?>" data-placeholder="<?php echo DIVI_HELPER::wpml_translate($taxonomy_info) ?>" multiple="" size="<?php echo($this->is_divi_use_chosen() ? 1 : '') ?>" name="<?php echo $tax_slug ?>">
    ...
    $terms = apply_filters('divi_sort_terms_before_out', $terms, 'mselect');
    ?>
    <?php if (!empty($terms)): ?>
        <?php foreach ($terms as $term) : ?>
            ...
            if (!empty($term['childs']))
            {
                divi_draw_mselect_childs($collector, $taxonomy_info, $term['term_id'], $tax_slug, $term['childs'], 1, $show_count, $show_count_dynamic, $hide_dynamic_empty_pos);
            }
